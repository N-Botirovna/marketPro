...
- **`shared/BookChatRow.jsx`** — ixcham **gorizontal card** (56–64px thumb + nom + narx + type badge tepa-o'ngda + egasining 📍 "Viloyat, Tuman"i past-o'ngda, ShopCard'dek border+hover). MUI. **Feed/browse** kontekstida (home `HomeBookList`, `CommunityBooksPage`, `ShopDetailPage`). Lokatsiya `utils/location.js:bookOwnerLocation()`'dan keladi (`posted_by.region_name/district_name` flat fieldlar — nested `region.name` YO'Q).
...
15. **JWT in localStorage (C-4)** — Phase 0'da mitigatsiya: CSP qattiqlandi (`unpkg.com` olib tashlandi, `dangerouslyAllowSVG=false`), Phase 1'da serverda logout endpoint refresh tokenni blacklist qiladi (FE `logoutUser()` shu yo'lga ko'chdi). **Phase 3 (2026-06): refresh-token endi HttpOnly cookie'da** (`COOKIE_REFRESH=true`, `src/config/index.js`) — XSS 14-kunlik tokenni o'g'irlay olmaydi. Access-token (90 daq) hali localStorage'da (cheklangan XSS oynasi). ⚠️ Cookie rejimda `refresh_token` localStorage'da YO'Q — sessiya mavjudligini tekshirish uchun `hasRefreshSession()` (`services/auth.js`) ishlatiladi (cookie rejimda `login_time` marker, body rejimda JS token). `getItem("refresh_token")`'ni to'g'ridan-to'g'ri **o'qimang** — cookie rejimda doim `null` (FE-H1/H2/H3 audit bug shu edi). To'liq httpOnly (access ham cookie + CSRF + async /me/ probe) hali deferred.
...
17. ~~**Kitob yaratish "zagruzka bo'lib qoladi"**~~ → (2026-06) Sabab: brauzer xom telefon fotosini (4–12 MB) siqmasdan yuborardi, backend esa rasmni so'rov ichida sinxron `ResizedImageField` (Pillow, 800×800) bilan o'lchaydi — mobil ulanishda upload 20s axios timeout'idan o'tib ketib, spinner "qotgandek" ko'rinardi, qayta urinish dublikat kitob yaratardi. Yechim (frontend lane): (a) **`src/lib/imageCompress.js`** — fayl yuklashdan oldin canvas orqali downscale+JPEG re-encode (1600px, q=0.82; <256KB yoki gif/svg o'tkazib yuboriladi; har qanday xatoda xom faylga fallback, hech qachon bloklamaydi); EXIF orientatsiya `createImageBitmap({imageOrientation:"from-image"})` bilan saqlanadi. (b) **Upload timeout 60s** — `services/books.js`'dagi `createBook`/`patchBook` `withIdempotency({ timeout: 60_000 })` bilan global 20s'ni override qiladi (nginx `proxy_read_timeout` + gunicorn `--timeout` bilan moslangan). **Yangi fayl-upload mutation qo'shsangiz** shu ikki narsani takrorlang: rasmni `compressImage()` bilan o'tkazing va service chaqiruviga `timeout` bering. Backend tomonda rasm resize'ini Celery'ga ko'chirish hali **deferred** (kattaroq, riskli o'zgarish; frontend siqishi uni asosan keraksiz qildi). **(c) Idempotency default-ON.** PROD nginx loglari ko'rsatdiki POST `/api/v1/book/`'ning ~25% i HTTP **499** (client javobni kutmay uzilgan) — kitob bazaga saqlanardi-yu, user "Tarmoq xatosi" ko'rardi, qayta urinish dublikat yaratardi. Backend allaqachon `idempotency-key`'ni CORS'da ruxsat beradi + `IdempotencyMiddleware` ishlaydi (prod'da tasdiqlandi), lekin `lib/idempotency.js` opt-in edi. Endi **default-ON** (`NEXT_PUBLIC_ENABLE_IDEMPOTENCY` faqat `0/false/off` bilan o'chiriladigan escape hatch) — har mutation `Idempotency-Key` yuboradi, `http.js` response interceptor network/timeout'da **bir xil kalit bilan retry** qiladi, backend cache'langan 2xx ni qaytaradi → dublikat yo'q, user success ko'radi. ⚠️ `NEXT_PUBLIC_ENABLE_IDEMPOTENCY` compose build-arg'lariga ulanmagan, shuning uchun **kod default'i** yagona to'g'ri joy edi. **(d) Xato endi toast'da** ham ko'rsatiladi (`BookCreateModal` submit catch/`success:false`) — ilgari xato faqat scroll qilinadigan kontent tepasidagi Alert'da edi, review bosqichida (pastda Save tugmasi) ko'rinmasdi. **(e) Review bosqichi qayta dizayn** — bitta telefon ekraniga sig'adigan: hero qator (muqova + nom/muallif/type/narx) + ixcham 2-ustunli detal grid; bo'sh maydonlar tushiriladi.
