...
const BookShop = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        setLoading(true);
        const [booksRes, categoriesRes, shopsRes, regionsRes] = await Promise.all([
          getBooks({ limit: 20 }),
  ...
  const filteredBooks = books.filter(book => {
    ...
    const matchesIsUsed = filters.is_used === "" || book.is_used === (filters.is_used === "true");
    const matchesType = !filters.type || book.type === filters.type;
    const matchesShop = !filters.shop || book.shop?.id === parseInt(filters.shop);
    
    const year = book.publication_year ? parseInt(book.publication_year) : 0;
    const matchesYearMin = !filters.publication_year_min || year >= parseInt(filters.publication_year_min);
    const matchesYearMax = !filters.publication_year_max || year <= parseInt(filters.publication_year_max);
                    ...
                    {categories.map((category) => (
                      <li key={category.id} className='mb-24'>
                        <button
                          ...
                          className={`text-gray-900 hover-text-main-600 text-start w-100 ${
                            filters.category === category.id.toString() ? 'text-main-600 fw-semibold' : ''
                          }`}
                        >
                          {category.name}
                        </button>
                   ...
                   <div className='position-relative'>
                     <ul className='max-h-300 overflow-y-auto scroll-sm'>
                       ...
                       {regions.length > 0 ? regions.map((region) => (
                         <li key={region.id} className='mb-16 position-relative'>
                           <button
                             onClick={() => {
                             ...
                             className={`text-gray-900 hover-text-main-600 text-start w-100 d-flex align-items-center justify-content-between ${
                               filters.region === region.id.toString() ? 'text-main-600 fw-semibold' : ''
                             }`}
                           >
                           ...
                           {hoveredRegionId === region.id &&
                             ...
                             region.districts.length > 0 && (
                               <div className='position-absolute top-0 start-100 ml-1 bg-white shadow-lg border rounded-2 p-16' style={{zIndex: 1000, minWidth: '200px'}}>
                                     ...
                                     <li key={district.id}>
                                       <button
                                         ...
                                         className={`text-gray-600 hover-text-main-600 text-start w-100 py-8 px-8 rounded-4 ${
                                           filters.district === district.id.toString() ? 'text-main-600 fw-semibold bg-main-50' : ''
                                         }`}
                                       >
                                         {district.name}
                                       </button>
                ...
                <div className='border border-gray-50 rounded-8 p-24'>
                  ...
                  <div className='d-flex gap-8'>
                    <input
                      ...
                      onChange={(e) => handleFilterChange('publication_year_min', e.target.value)}
                    />
                    <input
                      type='number'
                      className='common-input'
                      ...
                      onChange={(e) => handleFilterChange('publication_year_max', e.target.value)}
                    />
                  </div>
                </div>
